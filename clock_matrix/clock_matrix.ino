...
#include "LedControl.h"   // Control LED Matrix
...
// pin11 is connected to the CLK
// pin10 is connected to LOAD or CS
LedControl lc = LedControl(12, 11, 10, 4); // 4 LED Dot Matrix
...
// create an object of RTC_DS1307 module
RTC_DS1307 ds1307;          
...
// Start control LED Maxtrix from this function setup()
void setup() {
  ...
  // we have to initialize all devices in a loop
...
// All operations are lied in this function loop()
void loop() {
...
void basic_mode()
{
  ...
  while (run_mode()) {
    ...
    if (secs != rtc[0]) {
      ...
      // draw :
      plot (15, 2, 1); // top point
      plot (15, 5, 1); // bottom point
      count = 400;
    }

    // if count has run out, turn off the :
    if (count == 0) {
      plot (15, 2, 0); // top point
      plot (15, 5, 0); // bottom point
    }
    else {
      count--;
    }
...
void display_date()
{
  ...
  // get length of text in pixels, that way we can centre it on the display by divinding the remaining pixels b2 and using that as an offset
  byte len = 0;
